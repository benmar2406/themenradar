<script>
  ...
  let xAxisTicks = $derived(axisBottom(xScale)
    .ticks(tickCount)
  ...
  let yAxisTicks = $derived(axisLeft(yScale)
    .tickFormat(d => `${d}%`)
    .scale()
    .ticks()
    .map(t => ({
...
<figure class="item-container">
  ...
  <linearGradient id="gradientNegative" x1="0" y1="0" x2="0" y2="1">
    ...
    <stop offset="100%" stop-color="#FF7E1C" stop-opacity="0" />
        ...
        <!-- Areas -->
        <path d={areaPositive} fill="url(#gradientPositive)" stroke="#9DFF1C" stroke-width="1" in:draw={{ duration: 1200 }} />
        <path d={areaNeutral} fill="url(#gradientNeutral)" stroke="#FFF01C" stroke-width="1" in:draw={{ duration: 1200 }} />
        <path d={areaNegative} fill="url(#gradientNegative)" stroke="#FF7E1C" stroke-width="1" in:draw={{ duration: 1200 }} />
        ...
        <g transform={`translate(0, ${height - margin.bottom})`}>
          ...
          {#each xAxisTicks as tick}
            <g transform={`translate(${xScale(tick.value)}, 0)`}>
              ...
              <text y="20" text-anchor="middle" font-size="10" transform={innerWidth.current <= 1000 ? 'rotate(90 0 20)' : ''}>
                {tick.label}
              </text>
            </g>
          {/each}
